...
const getAllOrgUsers = async (req, res) => {
  try {
    ...
    if (!req.roles.includes('Admin')) {
      return res.status(403).json({ message: 'Access forbidden. You need to be an org Admin to perform this action.' });
    }

    // Retrieve all users with the role "User" in the specified organization (from query parameter)
    const orgUsers = await User.find({ organization: req.query.org, 'roles.User': { $exists: true } }).sort({ createdAt: -1 });
